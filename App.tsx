...
import { MinuteMenusLanding } from './components/landing/MinuteMenusLanding';

const App: React.FC = () => {
    const [mode, setMode] = useState<AppMode>(AppMode.CLONE);
    ...
    if (mode === AppMode.CLONE) {
        return <MinuteMenusLanding />;
    }
        ...
        <div className="min-h-screen bg-black flex items-center justify-center p-6 relative overflow-hidden selection:bg-white selection:text-black">
            ...
            <div className="max-w-4xl w-full relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                    ...
                    {/* Clover Clone - New Card */}
                    <button
                        onClick={() => setMode(AppMode.CLONE)}
                        className="md:col-span-2 group relative border border-zinc-800/80 p-8 text-center hover:bg-zinc-900 transition-all duration-500 hover:border-clover-green/50 flex flex-col items-center bg-zinc-900/30 backdrop-blur-sm"
                    >
                        <div className="w-12 h-12 bg-clover-green text-black rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                            <ArrowRight size={24} />
                        </div>
                        <h3 className="text-xl font-medium text-white mb-1">Clover Website Clone</h3>
                        <p className="text-zinc-500 text-xs leading-relaxed">
                            Preview the interactive website clone with custom fonts and styles.
                        </p>
                    </button>
                </div>
